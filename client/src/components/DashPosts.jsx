...
const DashPosts = () => {
    ...
    useEffect(() => {
        const fetchPosts = async () => {
            try {
                const res = await fetch(`https://blog-backend-ashen.vercel.app/api/post/getposts?userId=${currentUser._id}`);
    ...
    const handleShowMore = async () => {
        ...
        try {
            const res = await fetch(`https://blog-backend-ashen.vercel.app/api/post/getposts?userId=${currentUser._id}&startIndex=${startIndex}`)
            const data = await res.json()
            if (res.ok) {
    ...
    const handleDeletePost = async () => {
        setShowModal(false)
        try {
            const res = await fetch(`https://blog-backend-ashen.vercel.app/api/post/deletepost/${postIdToDelete}/${currentUser._id}`, {
                method: 'DELETE'
            })
            const data = await res.json()
            if (!res.ok) {
            ...
            <Modal show={showModal} onClose={() => setShowModal(false)} popup size="md">
                ...
                <Modal.Body>
                    <div className="text-center">
                        ...
                        <h3 className='mb-4 text-gray-500 text-lg dark:text-gary-400'>Are you sure to delete this post?</h3>
